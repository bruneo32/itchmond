...
var player = document.querySelector("itchmond-player");
if (player) {
	var __default_hwaccel = has_webgl_support()
	const itchmond_hwaccel = localStorage.getItem("itchmond/hwaccel")
	player._hwaccel = itchmond_hwaccel == null ? __default_hwaccel : (itchmond_hwaccel == "true" ? true : false)
	player.setHWAccel = function (e) {
		player._hwaccel = e
		localStorage.setItem("itchmond/hwaccel", e === true ? "true" : "false")
	...
	var ctx2d = screenCanvas.map((v) => {
		if (!player._hwaccel) {
			return v.getContext('2d', { alpha: false })
		} else {
			// Canvas with hardware acceleration
			const gl = v.getContext('webgl', { alpha: false })
			if (!gl) {
				console.error('WebGL not supported')
				// Fallback to default software canvas
				return v.getContext('2d', { alpha: false })
			}
			...
			var stride = 4 * Float32Array.BYTES_PER_ELEMENT; // 4 floats per vertex
			...
			// allocate initially an empty texture (so we can use texSubImage2D every frame)
			...
			// uniform location
			var uTexLoc = gl.getUniformLocation(prog, 'u_tex');
			gl.uniform1i(uTexLoc, 0); // texture unit 0

			// Allow pixelated scaling in CSS if desired:
			try { v.style.imageRendering = 'pixelated'; } catch (e) { }

			function resizeViewport() {
				// keep the canvas drawing buffer size in sync with CSS size
				var displayW = Math.max(1, v.clientWidth);
				var displayH = Math.max(1, v.clientHeight);
				if (v.width !== displayW || v.height !== displayH) {
					v.width = displayW;
					v.height = displayH;
				}
			...
			// Update function: feed ImageData (or Uint8Array) into GPU and draw
			function update(imageData, _dummy0, _dummy1) {
				// imageData is an ImageData object (FB[i])
				if (!imageData) return;
				resizeViewport();

				gl.activeTexture(gl.TEXTURE0);
				gl.bindTexture(gl.TEXTURE_2D, tex);
				// Upload pixels. We expect RGBA8 with width/height equal to 'width'/'height'
				// ImageData.data is a Uint8ClampedArray which works as an ArrayBufferView.
				gl.texSubImage2D(gl.TEXTURE_2D, 0, 0, 0, _WIDTH, _HEIGHT, gl.RGBA, gl.UNSIGNED_BYTE, imageData.data);
	...
	function emuRunFrame() {
		...
		// Update top screen pixels
		ctx2d[0].putImageData(FB[0], 0, 0)
		// Update bottom screen pixels
	...
	async function tryLoadROM(file) {
		...
		Module.HEAPU8.fill(0, romBufPtr, romBufPtr + romSize);
		...
		for (var i = 0; i < 2; i++) {
			if (!FB[i])
				...
				FB[i].data.set(new Uint8ClampedArray(Module.HEAPU8.buffer).subarray(fb + 256 * 192 * 4 * i, fb + 256 * 192 * 4 * (i + 1)))
		}
...
registerProcessor('my-worklet', MyAudioWorklet)`], { type: "text/javascript" }))).then(() => {
	...
	function handleTouch(event) {
		...
		// stickY returns a value from the bottom,
		// so convert it to the top
		stickY = window.innerHeight - stickY
		...
		for (var i = 0; i < event.touches.length; i++) {
			...
			var dom = t.target || document.elementFromPoint(t.clientX, t.clientY)
		...
		for (var k in vkState) {
			if (vkState[k][0] != vkState[k][1]) {
				...
				if (vkState[k][1]) {
					...
					if (k == "stick") {
						stickX = vkStickPos[1]
						stickY = window.innerHeight - vkStickPos[0]
						needUpdateStick = true
					}
				}
		...
		if (needUpdateStick) {
			// stickY returns a value from the top now,
			// so convert it to the bottom
			stickY = window.innerHeight - stickY

			const stick_center_y = stickY - stickH / 2
			const stick_center_x = stickX - stickW / 2
			vkMap['stick'].style.bottom = stick_center_y + 'px';
			vkMap['stick'].style.left = stick_center_x + 'px';
			vkMap['stick'].style.width = stickW + 'px';
			vkMap['stick'].style.height = stickH + 'px';
	...
	['touchstart', 'touchmove', 'touchend', 'touchcancel', 'touchenter', 'touchleave'].forEach((val) => {
		window.addEventListener(val, handleTouch, { passive: false })
	})


	// Mouse -> synthetic touch adapter (simple and compatible)
	function makeTouchFromMouse(e) {
		return {
			identifier: 0,
			target: e.target,
			force: 1,
	...
	['mousedown', 'mouseup', 'mousemove'].forEach((val) => {
		window.addEventListener(val, (e) => {
			...
			if (e.buttons & 1) {
				// Throw a fake touch event
				handleTouch({
					touches: [makeTouchFromMouse(e)],
					preventDefault: () => e.preventDefault(),
					stopPropagation: () => e.stopPropagation()
				})
			} else if (e.type == 'mouseup') {
				// Throw a fake touch up event
				handleTouch({
					touches: [],
					preventDefault: () => e.preventDefault(),
					stopPropagation: () => e.stopPropagation()
			...
			var isDown = (e.buttons == 1) && (isPointInRect(e.clientX, e.clientY, r))
	...
	function setScreenPos(c, left, top, w, h) {
		var sty = 'z-index:1;position:absolute;'
		sty += 'left:' + left + 'px;top:' + top + "px;width:" + w + "px;height:" + h + "px;"
	...
	function uiUpdateLayout() {
		isLandscape = (window.innerWidth / window.innerHeight) > 1.2
		// Top-bottom
		var maxWidth = window.innerWidth
		var maxHeight = window.innerHeight / 2
		var sz = maxScreenSize(maxWidth, maxHeight);
		var w = sz[0];
		var h = sz[1];
		var left = 0
